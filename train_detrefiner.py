...
class VectorDataset(Dataset):
    def __init__(self, train, dataset, clip_version, dino_version) -> None:
...
def train_and_evaluate(
    model, train_dataloader, evaluate_dataloader, criterion_train, criterion_val,
    epochs, lr, device, dataset, topk_per_image, score_thr, eval_every, use_ema, 
    W_DET, W_CLS, W_PATCH, grad_accum_steps, JSON_OUTPUT_DIR, JSON_LOAD_FILE, VERSION
):

    model.to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=0.01)
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / max(grad_accum_steps, 1))
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=max(epochs * num_update_steps_per_epoch, 1))
    ...
    def ema_update():
        ...
        with torch.no_grad():
            ...
            if ema_state is None:
                ema_state = {k: v.detach().clone() for k, v in msd.items()}
            else:
    ...
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            if idx % max(grad_accum_steps, 1) == 0:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                results = criterion_train(
                    class_vector, patch_vectors, batch["image_size"], 
                    batch["feat_b"].to(device), 
                    batch["text_labels_whole"], batch["text_labels_bboxes"],
                ...
                loss_class = results["class_vector_loss_main"]
                loss_patch = results["patch_vector_loss_main"]
                loss = loss_class + loss_patch

            scaler.scale(loss / max(grad_accum_steps, 1)).backward()

            do_update = ((idx + 1) % max(grad_accum_steps, 1) == 0) or ((idx + 1) == len(train_dataloader))
            if do_update:
            ...
            running_loss_patch += loss_patch.item()

        train_loss_class = running_loss_class / len(train_dataloader)
        train_loss_patch = running_loss_patch / len(train_dataloader)

        print(f"Epoch {epoch+1}/{epochs} - train_loss_class: {train_loss_class:.4f}")
        print(f"Epoch {epoch+1}/{epochs} - train_loss_patch: {train_loss_patch:.4f}")

        l1.append(train_loss_class)
        l2.append(train_loss_patch)

        # --- Evaluation (EMA weights) ---
        eval_state = None
        ...
        evaluate_loss_class = running_loss_class / len(evaluate_dataloader)
        evaluate_loss_patch = running_loss_patch / len(evaluate_dataloader)

        print(f"Epoch {epoch+1}/{epochs} - evaluate_loss_class: {evaluate_loss_class:.4f}")
        print(f"Epoch {epoch+1}/{epochs} - evaluate_loss_patch: {evaluate_loss_patch:.4f}")

        l3.append(evaluate_loss_class)
        l4.append(evaluate_loss_patch)
        ...
        if (epoch + 1) % eval_every == 0:
            torch.save(model.state_dict(), f'./detrefiner_{VERSION}_{epoch+1}.pth')

        # --- Re-score detections & Evaluate detection mAP every eval_every epochs ---
        if (epoch + 1) % eval_every == 0:
            model.eval()
    ...
    torch.save(model.state_dict(), f'./detrefiner_{VERSION}.pth')
    return l1, l2, l3, l4
...
if __name__ == "__main__":
    ...
    criterion_train = MultiGranularityContrastiveLoss(
        train=True, device=device, roi_mode=roi_mode,
    ...
    if dataset == "coco":
        topk_per_image = 300
        json_load_file = "results_coco_IDEA-Research_grounding-dino-tiny.json"
    elif dataset == "lvis":
        topk_per_image = 100000
        json_load_file = "results_lvis_IDEA-Research_grounding-dino-tiny.json"

    l1, l2, l3, l4 = train_and_evaluate(
        model, train_dataloader, evaluate_dataloader,
        criterion_train, criterion_val,
        epochs=30, lr=1e-3, device=device, dataset=dataset,
        topk_per_image=topk_per_image, score_thr=0.0, eval_every=5, use_ema=True, 
        W_DET=0.8, W_CLS=0.1, W_PATCH=0.1, grad_accum_steps=2, 
